class AudioController {
    ...
    failTimer = null;
    failStep = 0;
    muted = localStorage.getItem("riverRushMuted") === "1";
    init() {
        if (!this.ctx) {
    ...
    playAudioElement(element, volume = 1, startTime = 0) {
        this.init();
    ...
    startWaterAmbience() {
        if (!this.ctx || !this.master || this.waterSource)
            return;
        const duration = 2.5;
        ...
        this.melodyTimer = window.setInterval(() => {
            ...
            if (note) {
                ...
                if (this.melodyStep % 4 === 0)
                    this.playTone(note / 2, 0.28, "sine", 0.017);
            }
            this.melodyStep += 1;
        }, 310);
    }
    ...
    playBreath(startFrequency, endFrequency, duration, volume) {
        this.init();
    ...
    playTone(frequency, duration, type, volume, delay = 0, endFrequency) {
        this.init();
        if (!this.ctx || !this.master || this.muted)
            return;
    ...
    playNoise({ duration, volume, filter, attack, release }) {
        this.init();
        if (!this.ctx || !this.master || this.muted)
            return;
        const now = this.ctx.currentTime;
