...
export class PoseInputController {
    ...
    async start() {
        if (this.state.active || this.initializing)
            return;
    ...
    calibrateFromCurrentPose() {
        if (this.state.confidence < 0.45 || !this.currentBodyHeight)
            return false;
    ...
    update() {
        ...
        this.state.duckIntent = false;
        this.state.clapIntent = false;
        if (!this.state.active || !this.landmarker || this.video.readyState < 2)
            return this.state;
        ...
        if (!this.state.calibrated && !this.heightBaseline)
            this.heightBaseline = bodyHeight;
