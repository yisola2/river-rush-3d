...
function updateBotBodyAction(readings) {
  ...
  if (timeToImpact < 0.14 || timeToImpact > 0.58) return;

  if (nearest.type === OBSTACLE_TYPES.ROCK || nearest.type === OBSTACLE_TYPES.JUMP_GATE) startZAction("JUMPING");
  if (nearest.type === OBSTACLE_TYPES.LOG) startZAction("DUCKING");
}

function chooseOpenLane(readings) {
  const blocked = new Set();
  for (const reading of readings) {
    if (!reading.hit || reading.distance > 0.72) continue;
    let bestIndex = 0;
