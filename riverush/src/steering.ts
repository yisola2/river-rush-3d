...
type Behavior = {
  enabled: boolean;
  weight: number;
  apply: (context: BehaviorContext) => any;
};
...
export class Vehicle {
  ...
  constructor(config: SteeringConfig, x: number, z: number, maxSpeed = 7.6, maxForce = 22) {
    this.config = config;
  ...
  sync(x: number, velocityX: number) {
    this.position.x = x;
  ...
  seek(target) {
    ...
    if (desired.lengthSquared() === 0) return BABYLON.Vector3.Zero();
  ...
  flee(target, panicRadius = 2.2) {
    ...
    if (distance === 0 || distance > panicRadius) return BABYLON.Vector3.Zero();
    desired.normalize().scaleInPlace(this.maxSpeed);
  ...
  arrive(target, slowRadius = 3.6) {
    const desired = target.subtract(this.position);
    const distance = desired.length();
    if (distance === 0) return BABYLON.Vector3.Zero();
  ...
  limit(vector, max: number) {
    const length = vector.length();
    if (length <= max || length === 0) return vector.clone();
...
export class BehaviorManager {
  ...
  add(name: string, weight: number, apply: Behavior["apply"]) {
  ...
  setWeight(name: string, weight: number) {
    const behavior = this.behaviors.get(name);
    if (behavior) behavior.weight = weight;
  }
