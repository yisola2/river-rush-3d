...
export class World {
  ...
  update(dt: number) {
    ...
    this.scene.meshes.forEach((mesh) => {
      ...
      if (mesh.metadata.flow) {
        ...
        if (mesh.metadata.baseX !== undefined) mesh.position.x = mesh.metadata.baseX + Math.sin(this.riverOffset * 0.45 + phase) * sway;
        if (mesh.metadata.spin) mesh.rotation.y += dt * mesh.metadata.spin;
      }
    });
  }
  ...
  private createBankIrregularities() {
