...
export class Raft {
    ...
    updateModelAnimation(zState) {
        if (!this.modelRoot || zState === this.lastZState)
            return;
        ...
        if (this.activeAnimation.startsWith("jump:"))
            return;
        this.playAnimation("idle", true);
    }
    playAnimation(name, loop, speed = 1, onEnd) {
        const key = name.toLowerCase();
        const group = this.animationGroups.get(key) ?? [...this.animationGroups.entries()].find(([groupName]) => groupName.includes(key))?.[1];
        if (!group || this.activeAnimation === `${name}:${loop}`)
            return;
