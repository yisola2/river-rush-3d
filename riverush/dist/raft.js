...
export class Raft {
    ...
    async loadModel() {
        try {
            ...
            if (importedRoot)
                importedRoot.parent = this.modelRoot;
            result.meshes.forEach((mesh) => {
                mesh.isPickable = false;
                mesh.visibility = 1;
                if (mesh.material)
                    this.makeImportedMaterialReadable(mesh.material);
            });
    ...
    makeImportedMaterialReadable(material) {
    ...
    playIntro() {
        if (!this.modelRoot || this.introPlaying)
            return;
    ...
    playVictory() {
        if (!this.modelRoot || this.specialAnimation === "fall")
            return;
    ...
    updateModelAnimation(zState) {
        if (!this.modelRoot || this.specialAnimation || zState === this.lastZState)
            return;
        ...
        if (zState === "DUCKING") {
            this.playAnimation("stand_to_crouch", false, 1.6, () => {
                if (this.lastZState === "DUCKING" && !this.specialAnimation)
    ...
    playAnimation(name, loop, speed = 1, onEnd, startOffsetFrames = 0) {
        const group = this.findAnimationGroup(name);
        const activeKey = `${name}:${loop}:${startOffsetFrames}`;
        if (!group || this.activeAnimation === activeKey)
            return;
        this.animationSerial += 1;
        const serial = this.animationSerial;
        for (const anim of this.animationGroups.values())
            anim.stop();
        ...
        if (onEnd) {
            const observer = group.onAnimationGroupEndObservable.add(() => {
                ...
                if (serial === this.animationSerial)
                    onEnd();
            });
        }
    }
