import { JUMP_DURATION, PLAYER_RADIUS, RAFT_Z, RAY_ANGLES, RIVER_HALF_WIDTH, SENSOR_RANGE, STEERING_CONFIG, } from "./constants.js";
...
function createCollectEffect() {
    audio.playCoin();
    createOneShotParticles("collectSparkle", raft.root.position.add(new BABYLON.Vector3(0, 1.15, 0.15)), new BABYLON.Color4(1, 0.92, 0.25, 1), new BABYLON.Color4(1, 0.58, 0.08, 0.9), { capacity: 90, minSize: 0.1, maxSize: 0.34, minLifeTime: 0.22, maxLifeTime: 0.62, emitRate: 360, duration: 0.07 });
}
...
function createSplashEffect() {
    ...
    createOneShotParticles("splashJets", splashCenter, new BABYLON.Color4(0.86, 0.99, 1, 1), new BABYLON.Color4(0.35, 0.78, 1, 0.78), {
        capacity: 90,
    ...
    createOneShotParticles("splashCrown", splashCenter.add(new BABYLON.Vector3(0, 0.05, 0)), new BABYLON.Color4(0.75, 0.96, 1, 0.9), new BABYLON.Color4(0.18, 0.62, 0.92, 0.6), {
        capacity: 75,
...
function createDuckEffect() {
    audio.playDuck();
    createOneShotParticles("duckSpray", raft.root.position.add(new BABYLON.Vector3(0, 0.18, 0.55)), new BABYLON.Color4(0.65, 0.95, 1, 0.75), new BABYLON.Color4(0.22, 0.55, 0.82, 0.45), {
        capacity: 80,
...
function createPassEffect() {
    audio.playPass();
    createOneShotParticles("cleanPassSpray", raft.root.position.add(new BABYLON.Vector3(0, 0.28, -0.15)), new BABYLON.Color4(0.78, 1, 0.92, 0.75), new BABYLON.Color4(0.45, 0.85, 1, 0.5), {
        capacity: 70,
...
function updateSensors() {
    ...
    if (debugTargetMarker) {
        ...
        if (showTarget)
            debugTargetMarker.position.set(latestSteeringPlan.targetX, 0.18, RAFT_Z + 3.6);
    }
}
...
function toggleDebugMode() {
    ...
    if (!debugMode && debugTargetMarker)
        debugTargetMarker.visibility = 0;
}
function updateUi() {
    updateHtmlMenu();
