import { DESPAWN_Z, LANES, OBSTACLE_TYPES, PLAYER_RADIUS, RAFT_Z, RAY_ANGLES, RIVER_HALF_WIDTH, SENSOR_RANGE, SPAWN_Z, } from "./constants.js";
const WAVE_PATTERNS = [
    {
...
export class ObstacleSystem {
    ...
    showHitboxes = false;
    hitboxMaterial;
    constructor(scene, materials, assets, shadowGenerator) {
        this.scene = scene;
    ...
    checkCollisions(raftX, zState) {
        for (const obstacle of this.obstacles) {
            if (Math.abs(obstacle.z - RAFT_Z) > 1.05)
                continue;
            if (obstacle.type === OBSTACLE_TYPES.STAR) {
            ...
            if (!lateralHit)
                continue;
            if (obstacle.escapeActions.includes(zState))
                continue;
        ...
        return RAY_ANGLES.map((angle) => {
            ...
            for (const obstacle of this.obstacles) {
                if (obstacle.type === OBSTACLE_TYPES.STAR || obstacle.scored)
                    continue;
                if (obstacle.z < RAFT_Z || obstacle.z > RAFT_Z + SENSOR_RANGE)
                    continue;
    ...
    toggleHitboxes() {
        ...
        for (const obstacle of this.obstacles) {
            if (obstacle.hitbox)
                obstacle.hitbox.setEnabled(this.showHitboxes);
        }
    }
    pickWavePattern(distance) {
    ...
    resolvePatternLane(laneIndex, mirror) {
    ...
    createObstacleMesh(type, root) {
        if (type === OBSTACLE_TYPES.ROCK)
            return this.createRock(root);
        if (type === OBSTACLE_TYPES.LOG)
            return this.createLog(root);
        if (type === OBSTACLE_TYPES.STAR)
            return this.createStar(root);
    ...
    hasLateralOverlap(obstacle, raftX) {
        if (obstacle.type === OBSTACLE_TYPES.JUMP_GATE)
            return Math.abs(raftX) < RIVER_HALF_WIDTH - 0.3;
        return Math.abs(raftX - obstacle.x) < PLAYER_RADIUS + obstacle.radius;
    }
    createHitbox(type, radius, root) {
        let hitbox;
